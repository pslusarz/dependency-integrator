...
class Gradle {
    static File run(CarfaxGradleProjectSource projectSource, String command) {
        ...
        if (buildFile.exists()) {
            Command.run("${projectSource.projectDirectory.absolutePath}\\gradlew{$Command.isWindows()?'.bat':''} --build-file ${buildFile.absolutePath} --gradle-user-home ${projectSource.projectDirectory.absolutePath} ${command} >${output.absolutePath}")
        } else {
